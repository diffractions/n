...
public class GA implements Runnable {
	...
	public GA(int arr_length, int ind_count, double RMm, double Rmm, double ns, ThreadGroup tg, SortedSet<Ind> set,
			CountDownLatch cdl) {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		double[][] k = new double[][] { new double[] { 413.45, 0.155, 0.237 } };
	...
	public static Map<Double, Set<Ind>> start_try(double[][] k, int a, int b)
		...
		for (double[] t : k) {
			...
			for (int count = start_arr_length, length = start_arr_length; count <= start_arr_length
				...
				new Thread(tg, new GA(length, count, t[2], t[1], t[3], tg, set, cdl), "TN - " + count).start();
	...
	public static Map<Double, Set<Ind>> start_try(double[][] k) throws InterruptedException {

		return start_try(k, 5, 35);
	}
	...
	public void run() {
		...
		for (int r = 0; r < ind_count; r++) {
			Ind s = IndFactory.genRand(arr_length, xmin, xmax, nmin, nmax, ns, RMm, Rmm, rand);
			t.add(s);
		}
		...
		if (t.get(0).getFitness() < (error * 5) && t.get(0).getXf() < in_xmax && t.get(0).getNf() < in_nmax
				&& t.get(0).getXf() > in_xmin && t.get(0).getNf() > in_nmin)
			set.add(t.get(0));
	...
	private void generate(ArrayList<Ind> t) {
		for (int i = generaredFrom; i < generaredTo; i++) {
			...
			if (isRandom)
				sectionElement = (generaredFrom) + rand.nextInt(generaredTo - generaredFrom);
			t.set(sectionElement, IndFactory.genRand(arr_length, xmin, xmax, nmin, nmax, ns, RMm, Rmm, rand));
	...
	private void mutate(ArrayList<Ind> t) {
		for (int i = mutateFrom; i < mutateTo; i++) {
			int mutateElement = i;
			if (isRandom)
				mutateElement = (mutateFrom) + rand.nextInt(mutateTo - mutateFrom);

			t.set(mutateElement, IndFactory.invert(t.get(mutateElement), xmin, xmax, nmin, nmax, ns, RMm, Rmm));
		}
	}
	...
	private void crossover(ArrayList<Ind> t) {
		for (int i = crossoverFrom; i < crossoverTo; i++) {
			...
			if (isRandom)
				crossoverElement = (crossoverFrom) + rand.nextInt(crossoverTo - crossoverFrom);

			t.set(crossoverElement, IndFactory.crossover(t, crossoverElement - 1, crossoverElement, crossoverArrPos,
